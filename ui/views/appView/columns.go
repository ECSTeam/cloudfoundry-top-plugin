...
func (asUI *AppListView) columnLogStdout() *uiCommon.ListColumn {
	sortFunc := func(c1, c2 util.Sortable) bool {
		return c1.(*eventdata.AppStats).TotalLogStdout < c2.(*eventdata.AppStats).TotalLogStdout
	}
	displayFunc := func(data uiCommon.IData, isSelected bool) string {
		appStats := data.(*eventdata.AppStats)
...
func (asUI *AppListView) columnLogStderr() *uiCommon.ListColumn {
	sortFunc := func(c1, c2 util.Sortable) bool {
		return c1.(*eventdata.AppStats).TotalLogStderr < c2.(*eventdata.AppStats).TotalLogStderr
	}
	displayFunc := func(data uiCommon.IData, isSelected bool) string {
		appStats := data.(*eventdata.AppStats)
